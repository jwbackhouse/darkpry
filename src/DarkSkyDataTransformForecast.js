...
const getDarkSkyData = async (hostname, protocol, path, token, coordinate) => {
  ...
  const formattedRequestURLForCoordinateAsString = `${protocol}://${hostname}/${path}/${token}/${coordinate.lat},${coordinate.long}?exclude=flags,alerts,minutely,currently,hourly&units=si`
